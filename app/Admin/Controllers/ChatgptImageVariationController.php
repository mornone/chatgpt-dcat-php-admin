...
/**
 * 编辑图片
 * Class ChatgptImageVariationController.
 */
class ChatgptImageVariationController extends AdminController
{
    protected $view = 'admin.chat.image_variation';
    ...
    public function process(Request $request): void
    {
        ...
        try {
            ...
            $data = $open_ai->createImageVariation($opts);

            if (! empty($data)) {
                log::info('图片变体请求用户:' . $requestUid . '消息:' . json_encode($opts, 256) . '回复:' . $data);
                ...
                ChatgptApiRequestLog::create([
                    'user_id' => $requestUid,
                    'type' => ChatgptApiRequestLogType::IMAGE_VARIATION,
            ...
            log::info('图片变体请求用户:' . $requestUid . '出现异常文件：' . $e->getFile() . '。行号:' . $e->getLine() . '。错误:' . $e->getMessage());
